...
export const getPersonaResponse = (type: 'success' | 'error' | 'confirm' | 'dancing' | 'greeting', details?: string) => {
    ...
    switch (type) {
        case 'success': return getRandom(successPhrases);
        case 'error': return getRandom(errorPhrases);
        case 'confirm': return getRandom(confirmPhrases);
        case 'dancing': return getRandom(dancePhrases);
        case 'greeting': return "Systems online. What the fuck are we building today, boss man?";
...
export const checkSlangConfirmation = (text: string): 'yes' | 'no' | 'modify' | 'unknown' => {
    ...
    // Affirmative Slang
    if (t.match(/(run that shit|fuck ya|fuck yeah|do it|ship it|let's go|yes|confirm|looks good|love it)/)) {
        return 'yes';
    }
