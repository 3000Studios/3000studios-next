...
export async function executeUpdateText(newText: string): Promise<ExecutionResult> {
    try {
        ...
        if (headlinePattern.test(content)) {
            content = content.replace(headlinePattern, `<h1 className="text-6xl font-bold mb-6">${newText}</h1>`);
...
export async function executeAddMedia(
    ...
    try {
        ...
        const mediaComponent = type === 'video'
 ...
 */
export async function executeAddSection(
    title: string,
    content: string
): Promise<ExecutionResult> {
    try {
  ...
  return (
    <article className="max-w-4xl mx-auto py-24 px-6">
      ...
      <div className="prose prose-invert prose-lg max-w-none">
        ${body.split('\n\n').map(para => `<p className="mb-4 text-gray-300 leading-relaxed">${para}</p>`).join('\n        ')}
      </div>
    </article>
  );
}
