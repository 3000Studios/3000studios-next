...
import { QueuedMutation } from './types'; // Assuming types if exist, or I define interface here
...
export async function processPendingMutations(): Promise<QueuedMutation[]> {
  ...
  const queue = await readQueue();
  console.log(`Read queue, found ${queue.length} items. Pending: ${queue.filter(m => m.status === 'pending').length}`);
  let updated = false;
...
async function applyMutation(mutation: QueuedMutation): Promise<void> {
  ...
  switch (mutation.type) {
    ...
    case 'ADD_SECTION': {
      ...
      const sectionContent = String(mutation.payload?.sectionContent ?? 'Autogenerated by 3KAI voice command.');
      await handleAddSection({ payload: { page: 'page', component: sectionTitle, sectionContent } });
      return;
    }
    ...
    case 'ADD_MEDIA': {
      ...
      await handleAddMedia({ payload: { page, url: mediaUrl, kind: mediaType } });
      return;
    }

    case 'CHANGE_STYLE': {
      const cssVariable = String(mutation.payload?.cssVariable ?? mutation.payload?.property ?? '--accent-base');
      const value = String(mutation.payload?.value ?? '#D4AF37');
      console.log(`Calling handleChangeStyle for ${cssVariable} -> ${value}`);
      await handleChangeStyle({ payload: { target: cssVariable.startsWith('--') ? cssVariable : `--${cssVariable}`, value } });
      return;
    }

    case 'PUBLISH_BLOG': {
      const title = String(mutation.payload?.title ?? mutation.payload?.topic ?? 'Voice Automation Update');
      const body = String(mutation.payload?.body ?? 'This post was published via 3KAI voice automation.');
      const slug = mutation.payload?.slug ? String(mutation.payload.slug) : undefined;
      const blogPayload = { topic: title, body, slug };
      await handlePublishBlog({ payload: blogPayload });
      return;
    }
