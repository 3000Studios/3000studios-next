...
const commandPatterns: CommandHandler[] = [
  ...
  {
    pattern:
      /(?:update|change)\s+(?:the\s+)?homepage\s+(?:hero|headline)\s+(?:to\s+)?(.+)/i,
    category: "content",
  ...
  {
    pattern:
      /change\s+(?:the\s+)?price\s+(?:of\s+)?(.+?)\s+to\s+\$?(\d+(?:\.\d{2})?)/i,
    category: "store",
  ...
  {
    pattern:
      /show\s+(?:me\s+)?(?:the\s+)?revenue\s+(?:for\s+)?(?:today|this\s+month|this\s+week)/i,
    category: "system",
    action: "show_revenue",
    handler: (match) => ({
      category: "system",
...
export function validateCommand(
  ...
  if (
    ...
    !["admin", "pro", "godMode"].includes(userRole)
  ) {
