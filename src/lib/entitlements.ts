...
export class EntitlementManager {
  ...
  createEntitlements(userId: string, tier: string = "free"): UserEntitlements {
    ...
    const entitlements: UserEntitlements = {
      ...
      subscriptionExpiresAt:
        tier !== "free" ? Date.now() + 30 * 24 * 60 * 60 * 1000 : undefined,
      features,
    };
  ...
  hasFeature(userId: string, feature: string): boolean {
    ...
    if (
      ...
      entitlements.subscriptionExpiresAt < Date.now()
    ) {
      if (
        !entitlements.gracePeriodExpiresAt ||
        entitlements.gracePeriodExpiresAt < Date.now()
      ) {
