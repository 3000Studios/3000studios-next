      ...
      <form onSubmit={submit} className="w-full max-w-lg space-y-4 card">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Affiliate, listing fee, or dropship — submit your feed and we’ll
            review.
          </p>
        </div>
        ...
        <button
          disabled={loading}
          className="w-full py-3 rounded bg-[var(--electric-blue)] text-black font-bold hover:opacity-90 disabled:opacity-50"
        >
          {loading ? "Submitting..." : "Submit"}
        </button>
        {status && (
          <div className="text-sm text-[var(--electric-blue)]">{status}</div>
        )}
      </form>
    </div>
  );
