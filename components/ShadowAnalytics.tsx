...
export default function ShadowAnalytics() {
  ...
  const StatCard = ({ title, value, unit }: { title: string; value: string | number; unit?: string }) => (
    <div className="bg-corporate-navy border border-corporate-steel rounded-lg p-4">
      <h4 className="text-corporate-silver text-xs font-medium uppercase tracking-wide mb-2">{title}</h4>
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xl font-heading font-bold text-corporate-gold">System Analytics</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Avg Response" value={analytics.avgResponseTime} unit="ms" />
        <StatCard title="Active Connections" value={analytics.activeConnections} />
      </div>
